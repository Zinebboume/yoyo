...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.3, rectangle_colors=''):
    original_image = cv2.imread('C:/Users/dsben/OneDrive/Source/Source/Comptage_De_Vache/Projet_Finale_Comptage_De_Vache/TensorFlow-2.x-YOLOv3/IMAGES/25.jpg')
...
############################################################### fonctionalité de comptage ######################################
    list2 = []
    list2.append(len(bboxes))
    mylist4 = list2[-1]
    # print(mylist4)
    # CreateXMLfile("XML_Detections", str(int(time.time())), original_frame, bboxes, read_class_names(CLASSES))
    image = cv2.putText(image, 'Total:' + str(mylist4), (0, 30),
...
# detect from webcam
def detect_video_realtime_mp(video_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.3, rectangle_colors='', realtime=False):
    if realtime:
...
def detect_realtime(Yolo, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.3, iou_threshold=0.45, rectangle_colors=''):
    ...
    while True:
        ...
        bboxes = nms(bboxes, iou_threshold, method='nms')
        #print('le nombre de vaches est :', len(bboxes))
        #print(type(bboxes))
        #print(type(len(bboxes)))
        times.append(t2-t1)
        times = times[-20:]
...
####################################### Fonctionalité de comptage ##########################################################
        ...
        #print(mylist4)
        # CreateXMLfile("XML_Detections", str(int(time.time())), original_frame, bboxes, read_class_names(CLASSES))
        image = cv2.putText(frame, 'Total:'+ str(mylist4), (0, 30),
                          cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 255), 2)
